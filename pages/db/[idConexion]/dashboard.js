...
export async function getStaticProps(ctx) {
    ...
    const infoTables = await axios.post("http://localhost:3000/api/fastcrud/showTables", {
        idConexion
    });
    const promises = infoTables.data.tablas.map(async (tabla) => {
        const info = await axios.post("http://localhost:3000/api/fastcrud/getTableInfo", {
            idConexion,
            tabla
        });
        return tabla = {
...
export async function getStaticPaths() {
    const { data } = await axios.get("http://localhost:3000/api/fastcrud/getAllConections");
